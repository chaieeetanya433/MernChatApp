...
const newGroupChat = TryCatch(async (req, res, next) => {
  ...
  //this event is to give alert to all the participants in a group
  emitEvent(req, ALERT, allMembers, `welcome to ${name} group`);
  //this event is to fetch the latest chats when you join a group
...
const getMyChats = TryCatch(async (req, res, next) => {
  ...
  const transformedChats = chats.map(({ _id, name, members, groupChat }) => {
    //gives you members in your friend list
...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  //if the chat creator is not the one who wants to add
...
const removeMember = TryCatch(async (req, res, next) => {
  ...
  //if the chat creator is not the one who wants to remove
  if (chat.creator.toString() !== req.user.toString())
...
const renameGroup = TryCatch(async (req, res, next) => {
  ...
  //if you're not an admin, you cannot rename the group
  if (chat.creator.toString() !== req.user.toString())
    return next(new ErrorHandler("You are not allowed to add members", 403));
